use mongodb::{Client, Collection, error::Result, bson::{doc, oid::ObjectId}};
use std::{env, sync::Arc};

use crate::models::{user_model::User, room_model::Room, participant_model::Participant};
...
impl Database {
    pub async fn init() -> Result<Self> {
        ...
        Ok(Database { user, room, participant })
    }

    pub async fn get_user_by_id(db: Arc<Database>, user_id: ObjectId) -> mongodb::error::Result<Option<User>> {
    ...
    pub async fn create_room(db: Arc<Database>, host_id: ObjectId, code: String) -> mongodb::error::Result<()> {
    ...
    pub async fn get_room_by_code(db: Arc<Database>, room_code: &str) -> mongodb::error::Result<Option<Room>> {
    ...
    pub async fn add_participant_to_room(db: Arc<Database>, room_code: &str, user_id: ObjectId) -> mongodb::error::Result<()> {
    ...
    pub async fn add_participant(db: Arc<Database>, room_code: String, user_id: ObjectId) -> mongodb::error::Result<()> {
